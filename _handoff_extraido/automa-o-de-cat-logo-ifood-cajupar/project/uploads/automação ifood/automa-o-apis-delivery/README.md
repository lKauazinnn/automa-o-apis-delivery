...
| `VIEWER_ORIGIN` | não | Origem liberada no CORS do backend. Padrão: `http://localhost:5173`. |
...
| `CONVITE_REDIRECT_URL` | não | Pra onde o e-mail de convite manda a pessoa definir a senha. Padrão: mesmo valor de `VIEWER_ORIGIN`. |
...
| `POST /api/usuarios/convidar` | administrador | Convida alguém por e-mail (`{email, nome, papel}`). |
| `PATCH /api/usuarios/<id>/papel` | administrador | Troca o papel de alguém. |
| `POST /api/usuarios/<id>/resetar-senha` | administrador | Gera uma senha nova pra alguém direto (sem e-mail) e devolve ela na resposta pro admin repassar por fora. |
...
Autenticação de verdade via Supabase Auth — login é obrigatório pra usar o painel. Só existe
convite por e-mail (sem cadastro aberto): um administrador convida pelo modal "Usuários", a
pessoa recebe um link, define a senha e já entra logada com o papel que foi escolhido no convite.

**Bootstrap do primeiro administrador** (só uma vez, manual, porque ainda não existe nenhum
usuário no sistema pra convidar o primeiro):
1. Rode `sql/001_criar_tabela_auditoria.sql` e `sql/002_criar_tabela_perfis.sql`, nessa ordem, no
   SQL Editor do projeto Supabase.
2. Peça pra alguém com acesso ao backend (ou rode você mesmo com a `service_role` key) chamar
   `POST {SUPABASE_URL}/auth/v1/invite` com `{"email": "...", "data": {"nome": "...", "papel": "administrador"}}` —
   o trigger do banco já cria o perfil com esse papel. (Alternativa sem chamar a API: convide pelo
   Dashboard do Supabase em Authentication > Users, e depois rode
   `update public.perfis set papel = 'administrador' where email = '...'` no SQL Editor.)
3. A partir daí, esse administrador convida todo mundo pelo próprio painel.
