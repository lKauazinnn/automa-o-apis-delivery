  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border p-6" style={{ background: C.cardBg, borderColor: C.cardBorder }}>
          {children}
        </div>
        <p className="text-center text-[11px] mt-5" style={{ color: C.text3 }}>
...
const estiloInput = (C) => ({ background: C.inputBg, color: C.text1, border: `1px solid ${C.cardBorder}` })

function AvisoErro({ children, C }) {
  return (
    <p
      className="text-xs rounded-lg px-3 py-2 mb-3.5"
      style={{ color: C.bad, background: C.badBg, border: `1px solid ${C.badBd}` }}
    >
      {children}
    </p>
  )
...
function PainelEsqueciSenha({ onVoltar, C }) {
  ...
  if (enviado) {
    return (
      <div className="flex flex-col items-center gap-3 text-center py-2">
        <CheckCircle2 size={28} style={{ color: C.good }} />
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
      ...
      <div className="relative">
        <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: C.text2 }} />
        <input
          className={inputCls}
          style={estiloInput(C)}
          type="email"
      ...
      {erro && <AvisoErro C={C}>{erro}</AvisoErro>}
      <button
        type="submit"
        disabled={carregando}
      ...
      <button type="button" onClick={onVoltar} className="text-[11px] text-center" style={{ color: C.text2 }}>
        Voltar pro login
      </button>
    </form>
  )
...
export function TelaLogin({ onLogar, avisoInicial, C }) {
  const [email, setEmail] = useState('')
  ...
  return (
    <Casca C={C}>
      ...
      <h1 className="text-lg font-bold mb-5" style={{ color: C.text1 }}>
        {modoRecuperar ? 'Recuperar senha' : 'Entrar'}
      </h1>

      {avisoInicial && !modoRecuperar && <AvisoErro C={C}>{avisoInicial}</AvisoErro>}

      {modoRecuperar ? (
        <PainelEsqueciSenha onVoltar={() => setModoRecuperar(false)} C={C} />
      ) : (
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          <div className="relative">
            <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: C.text2 }} />
          ...
          <div className="relative">
            <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: C.text2 }} />
            <input
              className={inputCls}
              style={estiloInput(C)}
              type="password"
...
export function TelaDefinirSenha({ accessToken, eyebrow, aviso, onDefinida, onSair, C }) {
  const [senha, setSenha] = useState('')
  ...
  return (
    <Casca C={C}>
      ...
      <h1 className="text-lg font-bold mb-2" style={{ color: C.text1 }}>
        Defina sua senha
      </h1>
      {aviso && (
